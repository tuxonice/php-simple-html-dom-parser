...
// Debugging function to dump a single dom node with a bunch of information about it.
    function dump_node($echo = true)
    {
        ...
        if (isset($node->_[Constants::HDOM_INFO_INNER])) {
            $string .= $node->_[Constants::HDOM_INFO_INNER] . "'";
        } else {
            $string .= ' NULL ';
        }
...
// render begin tag
        if ($this->dom && $this->dom->nodes[$this->_[Constants::HDOM_INFO_BEGIN]]) {
...
//  $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-:]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
        ...
        foreach ($matches as $m) {
            ...
            if ($this->dom->lowercase) {
                $tag = strtolower($tag);
                $key = $key === null ? null : strtolower($key);
            }
